...
const char * const BITCOIN_CONF_FILENAME = "prasicoin.conf";
...
#else
    const char* pszModule = "prasicoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Prasicoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Prasicoin";
#else
    // Unix
    return pathRet / ".prasicoin";
#endif
#endif
}
